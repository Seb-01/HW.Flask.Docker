...
from app import app
from flask import jsonify, request, url_for, make_response, json, Response
...
def adverts_list():
    ...
    for ad in adverts:
        if ad.author == user or user.is_admin:
            advert=[]
            advert.append({'advert title': ad.title})
            advert.append({'advert body': ad.body})
            adverts_resp.append({ad.id: advert})
    json_response = json.dumps(adverts_resp, ensure_ascii=False)
...
def get_advert_by_id(id):
    ...
    if advert.author == user or user.is_admin:
        my_list = []
        my_list.append({ 'advert title': advert.title})
        my_list.append({ 'advert body': advert.body })
        data = {"result": my_list}
...
@app.route('/index')
def index():
    return jsonify({ 'index': 'Welcome in Adverdboard!' })